...
function SingleNotice() {
  ...
  const toggleAccordion = () => {
    ...
    return (
        <div className='flex space-x-1 md:space-x-2 font-medium font-serif text-[10px]  sm:text-xs md:text-base bg-white rounded border-black shadow-sm shadow-gray-500 mx-2 py-1 w-[85%] cursor-pointer' onClick={toggleAccordion}>
            {/* this is save icon */}
            <div className='flex mt-1 justify-center w-[10%] sm:w-[5%] pl-1 border-black text-center'>
            {isOpen ? <GiCancel className=' sm:text-base text-xs '/> : <FaBookmark />}
            </div>

            {/* this is the notice data or content */}
            <div class={`w-[70%] sm:w-[85%] border-black transition-max-h duration-500 ${isOpen ? '' : 'overflow-hidden overflow-ellipsis whitespace-nowrap'}`}  >
...
// {`w-[75%]  border-black   ${isFocused ? '' : 'overflow-hidden overflow-ellipsis whitespace-nowrap'}`}